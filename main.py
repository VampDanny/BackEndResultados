...
def getVotosPartidos(id_partido):
    json = miControladorResultado.getVotosPartidos(id_partido)
    return jsonify(json)

#Buscar Total Votos x Mesas
@app.route("/resultados/mesas", methods=["GET"])
def getVotosMesas():
    json = miControladorResultado.getVotosMesas()
    return jsonify(json)
